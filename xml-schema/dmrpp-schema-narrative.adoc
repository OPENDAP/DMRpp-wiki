...
This document describes the DMR++ `dmrpp` XML namespace and its elements. It is intended to be the start of a
specification for the DMR++.
...
`chunks`, `chunkDimensionSizes` and `chunk` elements, it is possible for a
...
* `compressionType`: a space-separated list of filters, not limited to
  ...
  32-bit hash of the data. _The order of the filters in the list is important._
...
* And one of:
  `dmrpp:chunk` elements, which is the typical case for an HDF5/NetCDF4 file.
  `dmrpp:block` elements, for example HDF4 block storage.
  A multi linked-block chunk arrangement where `dmrpp:chunk` elements refer to
  multiple underlying _blocks_. This case deals with formats where _chunks_ are
  not always atomic, such as HDF4.
...
In Example 1., the DMR `<Dataset>` element lacks the `dmrpp:href` attribute but
the `dmrpp:chunk` elements supply the needed information using that elements
optional `href` attribute. In this example, all the chunks use the same
...
._Example 3. How Fixed Length Strings are Represented_
[source,xml]
----
<?xml version="1.0" encoding="ISO-8859-1"?>
<Dataset xmlns="http://xml.opendap.org/ns/DAP/4.0#" 
    xmlns:dmrpp="http://xml.opendap.org/dap/dmrpp/1.0.0#"
    dapVersion="4.0" dmrVersion="2.0" name="oco2_Ephem_63191_260519_B11220_260519154035.h5"
    dmrpp:href="https://data....260519154035.h5"
    dmrpp:s3="..."
    dmrpp:s3credentials="https://data.gesdisc.earthdata.nasa.gov/s3credentials" dmrpp:trust="true"
    dmrpp:version="3.21.1-1090">
    <Attribute name="gesdisc_collection" type="String">
        <Value>11</Value>
    </Attribute>
        ...
    <Group name="Ephemeris">
        ...
        <String name="EquatorCrossingTime">
            <Dim size="1"/>
            <Attribute name="Description" type="String">
                <Value>The time of the equator crossing of the spacecraft ground track in the descending direction.</Value>
            </Attribute>
            <Attribute name="Shape" type="String">
                <Value>Scalar</Value>
            </Attribute>
            <Attribute name="Type" type="String">
                <Value>FixLenStr</Value>
            </Attribute>
            <dmrpp:chunks compressionType="shuffle deflate" deflateLevel="9" fillValue="">
                <dmrpp:chunkDimensionSizes>1</dmrpp:chunkDimensionSizes>
                <dmrpp:chunk offset="20212" nBytes="22" chunkPositionInArray="[0]"/>
            </dmrpp:chunks>
            <dmrpp:FixedLengthStringArray string_length="14" pad="null_term"/>
        </String>
    </Group>
</Dataset>
----
...
._Example 4. How Varying Length Strings are Represented_
[source,xml]
----
<?xml version="1.0" encoding="ISO-8859-1"?>
<Dataset xmlns="http://xml.opendap.org/ns/DAP/4.0#" 
    ...
    <Group name="Ephemeris">
       ...
       <String name="InputPointer">
            <Dim size="3"/>
            <Attribute name="Description" type="String">
                <Value>A pointer to one or more data granules that provide the major input that was used to generate this product.</Value>
            </Attribute>
            <Attribute name="Shape" type="String">
            ...
            <dmrpp:vlsa>
                <v>oco2_Orbit_B11220_260514232543.txt</v>
                <v>R0620020TR32613915360700.RBD</v>
                <v>R0620020AS12613911324200.RBD</v>
            </dmrpp:vlsa>
        </String>
    </Group>
</Dataset>
...
== dmrpp Namespace Attributes on the Dataset Element
